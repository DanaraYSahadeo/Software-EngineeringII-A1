...
def get_all_users_json():
    """Return all users in JSON format without role."""
    users = User.query.all()
    return [{'id': user.id, 'username': user.username} for user in users]  # Exclude role
...
def update_user(id, username, role=None):
    """Update username and optionally role by id."""
    user = get_user(id)
    if user:
